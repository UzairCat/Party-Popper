...
import { BOARD, CLASSIC_SETTINGS, PRESETS, type PropertySettings } from '../../../shared/property-game'
import type { RoomSnapshot } from '../../../shared/protocol'
import { AvatarArt } from '../../components/player/AvatarArt'
import { Button } from '../../components/common/Button'
import { getColourHex } from '../../data/playerOptions'
import { getPropertySettings, returnToGameSelection, socket, startPropertyMatch, updatePropertySettings } from '../../lib/socket'
import './own-it.css'
...
export function OwnItSetup({ room, playerId, onRoom, onLeave, notify }: { room: RoomSnapshot; playerId: string; onRoom: (room: RoomSnapshot) => void; onLeave: () => void; notify: (message: string) => void }) {
  ...
  const canStart = connected.length >= 2 && connected.length <= 8 && connected.length === room.players.length

  useEffect(() => {
    const onSettings = (value: PropertySettings) => setSettings(value)
    socket.on('property:settings', onSettings)
    void getPropertySettings().then((result) => { if (result.ok) setSettings(result.data) })
    return () => { socket.off('property:settings', onSettings) }
  }, [])
  ...
  const preset = async (name: 'classic' | 'quick' | 'casual') => {
    if (!isHost || pending) return
    setPending(true)
    const result = await updatePropertySettings({ ...PRESETS[name] })
    setPending(false)
    if (result.ok) setSettings(result.data)
    else notify(result.error.message)
  }
  ...
  return <main className="oi-page oi-setup page-enter">
    ...
    <section className="oi-setup-layout">
      <div className="oi-config">
        ...
        {tab === 'settings' ? <div className="oi-config__body"><div className="oi-section-title"><div><span className="oi-kicker">01 / MAKE IT YOURS</span><h2>Rule presets</h2></div><span className="oi-tag">{settings.preset}</span></div><div className="oi-presets">{(['classic', 'quick', 'casual'] as const).map((name) => <button key={name} type="button" className={settings.preset === name ? 'is-active' : ''} disabled={!isHost || pending} onClick={() => void preset(name)}><strong>{name}</strong><small>{name === 'classic' ? 'The full experience' : name === 'quick' ? '60-minute race' : 'More money, more chaos'}</small></button>)}</div><div className="oi-section-title"><div><span className="oi-kicker">02 / HOUSE RULES</span><h2>Match settings</h2></div></div>{MAIN_FIELDS.map((field) => <SettingRow key={field.key} field={field} settings={settings} disabled={!isHost || pending} onChange={(key, value) => void change(key, value)} />)}{settings.endCondition === 'rounds' && <SettingRow field={{ key: 'maxRounds', label: 'Maximum rounds', min: 10, max: 100, step: 1 }} settings={settings} disabled={!isHost || pending} onChange={(key, value) => void change(key, value)} />}{settings.endCondition === 'time' && <SettingRow field={{ key: 'timeLimitMinutes', label: 'Time limit (minutes)', min: 15, max: 240, step: 1 }} settings={settings} disabled={!isHost || pending} onChange={(key, value) => void change(key, value)} />}</div> : null}
        {tab === 'advanced' ? <div className="oi-config__body">{ADVANCED.map((section) => <section key={section.heading}><div className="oi-section-title"><h2>{section.heading}</h2></div>{section.fields.map((field) => <SettingRow key={field.key} field={field} settings={settings} disabled={!isHost || pending} onChange={(key, value) => void change(key, value)} />)}</section>)}</div> : null}
        {tab === 'rules' ? <div className="oi-rules"><span className="oi-kicker">THE SHORT VERSION</span><h2>Get rich. Stay rich.</h2><div className="oi-rule-grid"><article><span>01</span><h3>Roll & roam</h3><p>Take turns rolling two dice and moving clockwise. Pass Start to collect cash.</p></article><article><span>02</span><h3>Claim the block</h3><p>Buy unowned properties. Complete a colour group to build houses and hotels.</p></article><article><span>03</span><h3>Collect the rent</h3><p>Land on someone else’s property and pay them. Trade or mortgage when cash runs low.</p></article><article><span>04</span><h3>Own the endgame</h3><p>Bankrupt your rivals—or lead in net worth when a round or time limit ends.</p></article></div><p className="oi-rules__note">The board has {BOARD.length} spaces. Every roll, purchase and payment is verified by the server.</p></div> : null}
      </div>
      <aside className="oi-setup-side"><div className="oi-party-card"><span className="oi-kicker">THE PLAYERS</span><h2>Your table <span>{connected.length}/8</span></h2>{room.players.map((player) => <div key={player.id} className={`oi-party-player ${!player.isConnected ? 'is-away' : ''}`}><span className="oi-avatar" style={{ '--player-color': getColourHex(player.colour) } as CSSProperties}><AvatarArt avatar={player.avatar} /></span><strong>{player.name}</strong>{player.id === room.hostId && <small>HOST</small>}{!player.isConnected && <small>AWAY</small>}</div>)}{connected.length < 2 && <p className="oi-helper">Invite one more player to start.</p>}{room.players.length > 8 && <p className="oi-helper">Own It! supports up to eight players.</p>}{room.players.length !== connected.length && <p className="oi-helper">Wait for everyone to reconnect before starting.</p>}</div><div className="oi-start-card"><span className="oi-kicker">READY WHEN YOU ARE</span><h2>Fortune favours<br />the bold.</h2>{isHost ? <Button size="large" disabled={!canStart || pending} isLoading={pending} onClick={() => void start()}>Start game <span aria-hidden="true">↗</span></Button> : <p>Waiting for the host to start the game…</p>}<button type="button" onClick={() => void back()} disabled={!isHost || pending}>← Back to games</button><button type="button" onClick={onLeave}>Leave room</button></div></aside>
    </section>
  </main>
}
